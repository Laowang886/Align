    ...
    {open && <div className={styles.workspaceMenu}>
      {workspaces.map((workspace) => <button key={workspace.id} className={workspace.id === currentWorkspace?.id ? styles.selectedWorkspace : ""} onClick={() => onSelect(workspace.id)}>
        {workspace.avatarUrl ? (
          ...
          <img className={styles.workspaceMenuAvatar} src={`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000"}${workspace.avatarUrl}`} alt="" />
        ) : <i className={styles.workspaceMenuAvatar}><Icon name={presetIcons[workspace.avatarPreset ?? ""] ?? "board"} size={13} /></i>}<span>{workspace.name}</span>{workspace.id === currentWorkspace?.id && workspace.currentUserRole === "OWNER" && <i className={styles.workspaceSelectedDot} title="Delete workspace" onClick={(event) => { event.stopPropagation(); onDelete(workspace); }}><Icon name="trash" size={15} /></i>}
      </button>)}
